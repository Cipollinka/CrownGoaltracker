...
const homePagesButtons = [
  {
    screen: 'Home',
    iconImage: require('../assets/icons/buttons/projectsIcon.png'),
    selectedIconImage: require('../assets/icons/selButtonIcons/projectsIcon.png'),
  },
  {
    screen: 'Focusing',
    iconImage: require('../assets/icons/buttons/focusingIcon.png'),
    selectedIconImage: require('../assets/icons/selButtonIcons/focusingIcon.png'),
  },
  {
    screen: 'Analysis',
    iconImage: require('../assets/icons/buttons/analysisIcon.png'),
    selectedIconImage: require('../assets/icons/selButtonIcons/analysisIcon.png'),
  },
  {
    screen: 'Settings',
    iconImage: require('../assets/icons/buttons/settingsIcon.png'),
    selectedIconImage: require('../assets/icons/selButtonIcons/settingsIcon.png'),
  },
];
...
const HomeScreen = () => {
  ...
  const [visibleExecutorsProjectId, setVisibleExecutorsProjectId] =
    useState(null);
  ...
  const validateDateDeadlineChange = text => {
    ...
    if (cleaned.length >= 8) {
      ...
      if (month < 1 || month > 12) {
        ...
        Alert.alert(
          'Error',
          `Invalid day for the selected month. Please enter a valid day (01-${daysInMonth(
            month,
            year,
          )}).`,
        );
  ...
  const addExecutor = () => {
    if (executorName !== '' && executorSurname !== '') {
      const newId =
        executors.length > 0
          ? Math.max(...executors.map(executor => executor.id)) + 1
          : 1;
      const newExecutor = {
        id: newId,
        name: executorName,
        surname: executorSurname,
      };
  ...
  const removeExecutor = async executorToRemove => {
    try {
      const updatedExecutors = executors.filter(
        execut => !(execut.id === executorToRemove.id),
      );
  ...
  const removeProject = async projectToRemove => {
    try {
      const updatedProjects = projects.filter(
        proj => !(proj.id === projectToRemove.id),
      );
  ...
  const filteredProjects = projects.filter(
    project => project.status === selectedProjectCategory,
  );
              ...
              <TouchableOpacity
                ...
                <Text
                  style={{
                    ...
                    color:
                      selectedProjectCategory === category ? 'black' : 'white',
                    fontWeight: 500,
                  }}>
                  {category}
                </Text>
              ...
              {filteredProjects.length === 0 ? (
                ...
                filteredProjects.map((project, index) => (
                  <Swipeable
                    key={project.id}
                    renderRightActions={() =>
                      renderRightProjectActions(project)
                    }
                    onSwipeableOpen={() =>
                      handleSwipeableProjectOpen(project.id)
                    }
                    onSwipeableClose={() =>
                      handleSwipeableProjectClose(project.id)
                    }>
                    <TouchableOpacity
                      onPress={() => {
                      ...
                      <TouchableOpacity
                        onPress={() => {
                          toggleExecutorsVisibility(project.id);
                          setVisibleExecutorsProjectId(project.id);
                        }}
                        style={{
                          width: '100%',
                        ...
                        {executorsVisibility[project.id] &&
                          project.id === visibleExecutorsProjectId && (
                            <View
                              style={{
                                marginTop: dimensions.height * 0.01,
            ...
            <TouchableOpacity
              ...
              <Text
                style={{
                  ...
                  color:
                    selectedScreen === buttn.screen ? '#FFDE59' : '#919191',
            ...
            <View
              ...
              <View
                ...
                {[1, 2, 3, 4, 5].map(crovvn => (
                  <TouchableOpacity
                    key={crovvn}
                    onPress={() => handleComplexityPress(crovvn)}>
                    <Image
                      source={
                        complexity >= crovvn
                      ...
                      style={{
                        textAlign: 'center',
                        width: dimensions.width * 0.139,
                        height: dimensions.width * 0.139,
                ...
                <View
                  ...
                  {executors.map((executor, index) => (
                    <Swipeable
                      ...
                      renderRightActions={() =>
                        renderRightExecutorsActions(executor)
                      }
                      onSwipeableOpen={() => handleSwipeableToOpen(executor.id)}
                      onSwipeableClose={() =>
                        handleSwipeableClose(executor.id)
                      }>
                      <View
                        style={{
                          width: dimensions.width * 0.93,
